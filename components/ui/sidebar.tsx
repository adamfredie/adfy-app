...
function Sidebar({ className = "", children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <aside className={`aduffy-sidebar ${className}`} {...props}>
      {children}
    </aside>
  );
}

function SidebarItem({ className = "", children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={`aduffy-sidebar-item ${className}`} {...props}>
      {children}
    </div>
  );
}
...
function SidebarGroup({ className = "", children, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return (
    <div className={`aduffy-sidebar-group ${className}`} {...props}>
      {children}
    </div>
  );
}
...
function SidebarRail({ className, ...props }: ComponentProps<"button">) {
  ...
  return (
    <button
      ...
      className={`hover:after:bg-sidebar-border absolute inset-y-0 z-20 hidden w-4 -translate-x-1/2 transition-all ease-linear group-data-[side=left]:-right-4 group-data-[side=right]:left-0 after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] sm:flex in-data-[side=left][data-state=collapsed]_&]:cursor-e-resize in-data-[side=right][data-state=collapsed]_&]:cursor-w-resize hover:group-data-[collapsible=offcanvas]:bg-sidebar group-data-[collapsible=offcanvas]:translate-x-0 group-data-[collapsible=offcanvas]:after:left-full [[data-side=left][data-collapsible=offcanvas]_&]:-right-2 [[data-side=right][data-collapsible=offcanvas]_&]:-left-2 ${className || ''}`}
      {...props}
    />
  );
}
...
function SidebarInset({ className, ...props }: ComponentProps<"main">) {
  return (
    <main
      ...
      className={`bg-background relative flex w-full flex-1 flex-col md:peer-data-[variant=inset]:m-2 md:peer-data-[variant=inset]:ml-0 md:peer-data-[variant=inset]:rounded-xl md:peer-data-[variant=inset]:shadow md:peer-data-[variant=inset]:peer-data-[state=collapsed]:ml-2 ${className || ''}`}
      {...props}
    />
  );
}
